(() => {
  ...
  const lightenSourceChannel = (value) => Math.min(255, Math.round(value + (255 - value) * SOURCE_LIGHTEN_MIX));
  ...
  const createSourceCanvas = () => {
    ...
    cells.forEach((cell) => {
      ...
      const red = lightenSourceChannel(pixels[offset]);
      const green = lightenSourceChannel(pixels[offset + 1]);
      const blue = lightenSourceChannel(pixels[offset + 2]);
      ...
      cell.depletionColor = `rgb(${red}, ${green}, ${blue})`;
    });
  };
  ...
  const configure = () => {
    ...
    surface.dataset.pixelParticleColorMode = "source-rgb-lightened";
    surface.dataset.pixelParticleColorLift = "towards-white-52";
  ...
  const drawDisturbance = (now) => {
    ...
    affected.forEach(({ cell, level }) => {
      ...
      context.fillStyle = cell.depletionColor || "rgba(255, 238, 196, 0.9)";
      context.fillRect(
