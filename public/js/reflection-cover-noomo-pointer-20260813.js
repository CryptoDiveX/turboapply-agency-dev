(() => {
  ...
  const VERSION = 'reflection-cover-noomo-pointer-20260813-1';
  ...
  const setup = (surface) => {
    ...
    const draw = (time) => {
      ...
      ctx.filter = 'none';
      surface.dataset.reflectionCoverState = (inside || surface.classList.contains('is-reflection-fullscreen')) ? 'playing' : 'idle';
